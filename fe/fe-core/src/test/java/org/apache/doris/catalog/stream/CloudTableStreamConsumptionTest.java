...
public class CloudTableStreamConsumptionTest extends TestWithFeService {
    ...
    public void testConsumptionViewReadsAuthoritativeCloudOffsets() throws Exception {
        ...
        List<TRow> localRows = new ArrayList<>();
        stream.fillTableStreamConsumptionInfo(localRows, "p1"::equals);
        Assertions.assertEquals(1, localRows.size());
        Assertions.assertEquals("p1", localRows.get(0).getColumnValue().get(3).getStringVal());
        ...
        try (MockedStatic<MetaServiceProxy> mockedProxy = Mockito.mockStatic(MetaServiceProxy.class)) {
            ...
            Mockito.verify(proxy, Mockito.times(3)).getTableStreamOffset(Mockito.any());
        } finally {
