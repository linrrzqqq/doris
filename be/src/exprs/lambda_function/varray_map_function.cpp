...
public:
    ...
    Status execute(VExprContext* context, const Block* block, const Selector* expr_selector,
        ...
        do {
            ...
            // Limit each lambda execution by both its byte budget and the runtime row ceiling.
...
private:
    static size_t _saturating_add(size_t lhs, size_t rhs) {
    ...
    size_t _calculate_lambda_batch_size(const VExprSPtr& lambda_expr, const DataTypes& data_types,
        ...
        for (const auto& lambda_data : lambda_datas) {
            estimated_lambda_bytes_per_row = _saturating_add(estimated_lambda_bytes_per_row,
                                                             lambda_data->get_max_row_byte_size());
        }
        if (has_row_dependent_captures) {
            estimated_lambda_bytes_per_row =
                    _saturating_add(estimated_lambda_bytes_per_row, sizeof(IColumn::ColumnIndex));
            for (int column_id : required_input_column_ids) {
                const auto& input_column = block->get_by_position(column_id).column;
                if (!is_column_const(*input_column) &&
                    !check_and_get_column<ColumnNothing>(input_column.get())) {
                    estimated_lambda_bytes_per_row = _saturating_add(
