...
Status AggFnEvaluator::prepare(RuntimeState* state, const RowDescriptor& desc,
    ...
    RETURN_IF_ERROR(_init_const_arguments());
...
// is not necessarily `VExpr::is_constant()`, but it is safe to evaluate during
// prepare as long as it does not reference runtime input columns.
static bool contains_runtime_input_ref(const VExprSPtr& expr) {
    if (expr->is_slot_ref() || expr->is_column_ref() || expr->is_virtual_slot_ref()) {
        return true;
    }
    for (const auto& child : expr->children()) {
        if (contains_runtime_input_ref(child)) {
            return true;
        }
    }
    return false;
...
Status AggFnEvaluator::_init_const_arguments() {
    ...
    for (const auto i : const_argument_indexes) {
        const auto& root = _input_exprs_ctxs[i]->root();
        if (contains_runtime_input_ref(root)) {
            return Status::InvalidArgument(
                    "Aggregate function {} requires argument {} to be a constant expression, but "
                    "got "
                    "{}",
                    _function->get_name(), i, root->expr_name());
        }
