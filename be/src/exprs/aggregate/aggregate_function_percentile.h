...
inline const IColumn* get_const_argument_nested_column(const ColumnsWithTypeAndName& arguments,
    ...
    if (index >= arguments.size() || !arguments[index].column) [[unlikely]] {
        throw Exception(ErrorCode::INTERNAL_ERROR,
                        "Aggregate function {} requires const argument {}", function_name, index);
    }
...
public:
    ...
    Status set_const_arguments(const ColumnsWithTypeAndName& arguments) override {
        ...
        const auto& quantile_column =
                *get_const_argument_nested_column(arguments, const_argument_indexes[0], get_name());
        ...
        if (const_argument_indexes.size() > 1) {
            const auto& compression_column = *get_const_argument_nested_column(
                    arguments, const_argument_indexes[1], get_name());
...
public:
    ...
    Status set_const_arguments(const ColumnsWithTypeAndName& arguments) override {
        const auto& quantile_column = *get_const_argument_nested_column(arguments, 2, get_name());
        this->_quantile =
...
public:
    ...
    Status set_const_arguments(const ColumnsWithTypeAndName& arguments) override {
        ...
        const auto& compression_column =
                *get_const_argument_nested_column(arguments, 3, get_name());
...
public:
    ...
    Status set_const_arguments(const ColumnsWithTypeAndName& arguments) override {
        const auto& quantile_column = *get_const_argument_nested_column(arguments, 1, get_name());
        _quantile = assert_cast<const ColumnFloat64&, TypeCheckOnRelease::DISABLE>(quantile_column)
                            .get_data()[0];
        RETURN_IF_CATCH_EXCEPTION(check_quantile(_quantile));
        return Status::OK();
...
public:
    ...
    Status set_const_arguments(const ColumnsWithTypeAndName& arguments) override {
        const IColumn* quantiles_column =
                get_const_argument_nested_column(arguments, 1, get_name());
        const auto* quantile_array = check_and_get_column<ColumnArray>(quantiles_column);
        if (quantile_array == nullptr) [[unlikely]] {
        ...
        if (offset_column_data.empty()) [[unlikely]] {
