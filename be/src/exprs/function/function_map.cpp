    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        auto merge_null_map = [&](const ColumnPtr& column, bool is_const) -> const IColumn& {
            if (const auto* null_col = check_and_get_column<ColumnNullable>(column.get())) {
                ...
                has_null = has_null || null_col->has_null();
...
private:
    /// Non-const key/value columns must contain input_rows_count rows. For every
    /// non-null result row, the key and value arrays must contain the same number
    /// of elements. A const array is broadcast from its single nested row.
