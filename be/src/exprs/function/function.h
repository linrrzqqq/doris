...
public:
    ...
    // Only used by agg_state wrapper functions. Normal aggregate functions use
    // IAggregateFunction::set_const_arguments directly.
    virtual Status set_const_arguments(const ColumnsWithTypeAndName& /*arguments*/) {
        return Status::OK();
    }

    // Only used by agg_state wrapper functions. Normal aggregate functions use
    // IAggregateFunction::get_const_argument_indexes directly.
