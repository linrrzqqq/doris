...
TEST(FunctionMapTest, map_from_arrays_nullable) {
    ...
    Block block;
    block.insert({ColumnNullable::create(make_int_array({1, 2, 99, 3, 4, 5}, {1, 2, 3, 5, 6}),
                                         std::move(key_null_map)),
                  nullable_array_type, "keys"});
    block.insert({ColumnNullable::create(make_int_array({10, 20, 990, 30, 40, 50}, {1, 2, 3, 5, 6}),
    ...
    EXPECT_EQ(nested.get_offsets()[3], 4);
    EXPECT_EQ(nested.get_offsets()[4], 5);
    EXPECT_EQ(get_nullable_int(nested.get_keys(), 2), 3);
    EXPECT_EQ(get_nullable_int(nested.get_values(), 2), 30);
}
